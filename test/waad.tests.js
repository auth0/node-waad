...
describe('query graph', function () {
  ...
  it('should fail if access_token is wrong', function (done) {
    var waad = new Waad({tenant: config.TENANTID});
    waad.getUserByEmail('wrong-token', config.TENANTID, 'nonexising@auth10dev.onmicrosoft.com', function(err) {
...
describe('query graph using token obtained with client credentials', function () {
  ...
  it('can get all users', function (done) {
    waad.getUsers(access_token2, config.TENANTDOMAIN, function(err, users) {
