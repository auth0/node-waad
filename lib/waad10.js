...
function Waad10(options) {
  ...
  if (!this.options.tenant) {
    throw new Error(
      'Must supply "tenant" id (16a88858-..2d0263900406) or domain (mycompany.onmicrosoft.com)'
    );
  }
...
Waad10.prototype.__request = function (options, callback) {
  ...
  request(
    ...
    function (err, resp, body) {
      ...
      if (options.singleEntity) return callback(null, array);

      if (!array && array.value && array.value.length === 0) return callback();
...
Waad10.prototype.__queryUser = function (id, qs, options, callback) {
  if (typeof options === "function") {
    callback = options;
    options = options || {};
  }
  ...
  this.__request(
    {
      url: this.baseUrl + this.options.tenant + "/users/" + encodeURI(id),
      qs: qs,
      singleEntity: true,
    },
    function (err, user) {
      ...
      if (!options || !options.includeGroups) return callback(null, user);
      ...
      options.includeNestedGroups
        ? _this.__queryNestedUserGroups(user.objectId, groupsCallback)
        : _this.__queryUserGroup(user.objectId, groupsCallback);
    }
  );
};
...
Waad10.prototype.getSingleUserByUpn = function (upn, options, callback) {
  return this.getUser(upn, options, function (err, users) {
    if (err) return callback(err);
    return callback(null, users);
  });
};

Waad10.prototype.getUserByProperty = function (
  propertyName,
  propertyValue,
  options,
  callback
...
Waad10.prototype.getUser = function (id, options, callback) {
  var qs = {};
  ...
  return this.__queryUser(id, qs, options, callback);
};

Waad10.prototype.getGroupsForUserByObjectIdOrUpn = function (
  objectIdOrUpn,
  callback
) {
